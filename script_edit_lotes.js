...
// 1. Add server actions to src/app/actions.ts
const actionsPath = 'src/app/actions.ts';
let actionsCode = fs.readFileSync(actionsPath, 'utf8');

if (!actionsCode.includes('updateIngresoVencimiento')) {
  actionsCode += `
...
// Add iv.id to getLotes select
pageCode = pageCode.replace(
  /SELECT\s+iv\.cod_art,/,
  'SELECT \n        iv.id,\n        iv.cod_art,'
);

// We need to import the new Client Component
if (!pageCode.includes('LotesTableClient')) {
  pageCode = pageCode.replace(
...
// Replace the table with the Client Component
...
export default function LotesTableClient({ initialLotes }: { initialLotes: any[] }) {
  ...
  const getSemaforoInfo = (dias: number) => {
    if (dias < 0) return { bg: 'bg-[#D9D9D9]', border: 'border-[#D9D9D9]', text: 'text-slate-800' };
    if (dias <= 60) return { bg: 'bg-[#FF0000]', border: 'border-[#FF0000]', text: 'text-white' };
    if (dias <= 180) return { bg: 'bg-[#E97132]', border: 'border-[#E97132]', text: 'text-white' };
    if (dias <= 270) return { bg: 'bg-[#FFC000]', border: 'border-[#FFC000]', text: 'text-white' };
    return { bg: 'bg-[#00B050]', border: 'border-[#00B050]', text: 'text-white' };
  };
              ...
              return (
                <tr key={lote.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 text-center">
                    <div className={\`w-6 h-6 rounded-full mx-auto \${semaforo.bg} shadow-sm border \${semaforo.border}\`}></div>
                  </td>
...
console.log("Tabla extraída y funciones de edición/eliminación agregadas.");
